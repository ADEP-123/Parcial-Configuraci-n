...
La app usaba `alert()` nativo del navegador para informar al usuario sobre acciones completadas, lo cual bloquea el hilo principal, rompe la experiencia visual y no respeta el tema de la aplicación.

#### Solución propuesta

![alt text](src/assets/falta_validaciones_1.png)
...
La app usaba `window.confirm()` nativo del navegador para confirma eliminar productos y finalizar la compra. Al igual que `alert()` bloquea el hilo principal, no respeta el tema de la aplicació y no permite personalizar el diseño ni el mensaje.

#### Solución propuesta

![alt text](src/assets/falta_validaciones_2.png)

- Crear `src/hooks/useConfirm.js` con lógica basada en `Promise`: retorna `true` o `false` según la elección del usuario, lo que permite usar `async/await` en los handlers sin callbacks anidados
- Crear `src/components/ConfirmModal.jsx` con overlay semitransparente, animación de entrada elástica, título, ícono contextual según variante (`danger`, `success`, `warning`) y botones Cancelar / Confirmar con colores de contexto
- Hacer clic en el overlay cierra el modal (equivale a cancelar) para una experiencia intuitiva
- Integrar `useConfirm` en `App.jsx` pasando `confirm` como prop `onConfirm` al `ShoppingCart`, con `z-index: 10000` para que quede por encima de los toasts
- Reemplazar `window.confirm()` en **Finalizar compra**: el mensaje muestra el total y la cantidad de productos en el carrito
- Reemplazar `window.confirm()` en **Eliminar producto**: al confirmar también dispara un toast `info` con el nombre del producto eliminado
- Los estilos usan los tokens CSS del tema para que funcione correctamente en dark y light mode
